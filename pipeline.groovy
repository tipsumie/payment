pipeline {
    ...
    environment {
        ...
        KUBECTL_HOME = '/Users/a677026/google-cloud-sdk/bin/kubectl'
        BUILD_DATE = new Date().format('yyyy-MM-dd')
        IMAGE_TAG = "${BUILD_DATE}-${BUILD_NUMBER}"
        IMAGE_NAME = 'payment' // Variable for the image name
        DOCKER_USERNAME = 'tipsukon25' // Variable for Docker Hub username
    ...
    stages {
        ...
        stage('Build Maven') {
            steps {
                checkout([$class: 'GitSCM', credentialsId: 'githubpwd', branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/tipsumie/payment.git']]])
        ...
        stage('Deploy to k8s') {
            steps {
                withKubeConfig([credentialsId: 'kubectlpwd', serverUrl: 'https://127.0.0.1:56806']) {
                    script {
